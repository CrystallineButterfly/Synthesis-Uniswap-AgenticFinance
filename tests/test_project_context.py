...
MODULE = importlib.import_module('agents.uniswap_agent')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'uniswap_agent')
        self.assertEqual(spec.primary_contract_name, 'UniswapExecutionGuard')
