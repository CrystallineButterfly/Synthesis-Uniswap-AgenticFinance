...
import {UniswapExecutionGuard} from "src/UniswapExecutionGuard.sol";

contract UniswapExecutionGuardTest is Test {
    UniswapExecutionGuard private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_uniswap_agenticfinance");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new UniswapExecutionGuard(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        UniswapExecutionGuard freshController = new UniswapExecutionGuard(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
